/**
 * TaskCardEditorTest - Simple test version for incremental development
 * 
 * This is a minimal implementation that works. We'll add features one by one.
 */
class TaskCardEditorTest {
    constructor(taskId) {
        this.taskId = taskId;
        this.universeData = {};
...
// Initialize all test editors on page load
document.addEventListener('DOMContentLoaded', function() {
    if (!window.taskCardEditorsTest) {
        window.taskCardEditorsTest = {};
    }
    
    // Find all test task edit forms
    document.querySelectorAll('.task-edit-form-simple[data-task-id]').forEach(form => {
        const taskId = parseInt(form.dataset.taskId);
        if (taskId && !window.taskCardEditorsTest[taskId]) {
            window.taskCardEditorsTest[taskId] = new TaskCardEditorTest(taskId);
        }
    });
