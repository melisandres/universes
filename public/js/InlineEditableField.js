window.InlineEditableField = {
    ...
    mounted() {
        ...
        // Auto-enter edit mode if autoEdit prop is true
        if (this.autoEdit) {
            this.enterEditMode();
        }
    },
    watch: {
        ...
        autoEdit(newValue) {
            // If autoEdit becomes true and we're not already editing, enter edit mode
    ...
    methods: {
        enterEditMode() {
            ...
            this.$nextTick(() => {
                const input = this.$el.querySelector('.inline-field-input');
                if (input) {
                    input.focus();
                    if (input.select) {
                        input.select();
